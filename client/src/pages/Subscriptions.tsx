...
export default function Subscriptions() {
  const { subscriptions, terminateSubscription } = useSavings();

  // Subscription Persistence: Only count active subscriptions (not terminated) in burn rate
  const totalBurnRate = subscriptions
  ...
  return (
    <div className="min-h-screen bg-zinc-950 pb-24">
      ...
      <main className="max-w-md mx-auto p-4 space-y-6">
        ...
        {(() => {
          // Subscription Persistence: Filter out terminated subscriptions from Active/Ghost lists
          // Terminated subscriptions are moved to Hidden/Terminated state
          const activeSubscriptions = subscriptions.filter((s) => 
            s.status === "active" && s.isGhost !== true && s.status !== "terminated"
          );
          const ghostSubscriptions = subscriptions.filter((s) => 
            (s.status === "ghost" || s.isGhost === true) && s.status !== "terminated"
          );
          ...
          return (
            <>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-green-500 font-medium">
                        {activeSubscriptions.length} {activeSubscriptions.length === 1 ? "target" : "targets"}
                      </span>
                    </div>
                  </div>
                        ...
                        <div className="relative p-5">
                          ...
                          <div className="flex items-start justify-between mb-4">
                            <div className="flex items-center gap-3">
                              <div className="w-12 h-12 rounded-lg border bg-zinc-800 border-zinc-700 text-cyan-500 flex items-center justify-center transition-transform duration-300 group-hover:scale-110">
                                <Skull className="w-6 h-6" />
                              </div>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-red-500 font-medium">
                        {ghostSubscriptions.length} {ghostSubscriptions.length === 1 ? "target" : "targets"}
                      </span>
                    </div>
                  </div>

                  <div className="space-y-4">
                    {ghostSubscriptions.map((subscription) => (
                      <motion.div
                        key={subscription.id}
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="relative overflow-hidden rounded-xl bg-zinc-900/50 border border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.3)] transition-all duration-300 group"
                      >
                        {/* Background Glow */}
                        <div className="absolute inset-0 bg-gradient-to-br from-red-500/5 to-transparent pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                        ...
                        <div className="relative p-5">
                          ...
                          <div className="flex items-start justify-between mb-4">
                            <div className="flex items-center gap-3">
                              <div className="w-12 h-12 rounded-lg border bg-red-950/50 border-red-500/50 text-red-400 flex items-center justify-center transition-transform duration-300 group-hover:scale-110">
                                <Ghost className="w-6 h-6" />
                              </div>
                              <div className="flex-1 min-w-0">
                                <h3 className="text-lg font-semibold text-zinc-100 group-hover:text-red-400 transition-colors break-words">
                                  {subscription.name}
                                </h3>
                                <div className="mt-1">
                                  <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-[0.15em]">
              ...
              {/* Empty State */}
              {activeSubscriptions.length === 0 && ghostSubscriptions.length === 0 && (
